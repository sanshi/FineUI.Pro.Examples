...
namespace FineUI.Pro.Examples.grid
{
    public partial class grid_edit_cart : PageBase
    {
        ...
        private void BindGrid()
        {
            ...
            Grid1.SelectedRowIDArray = new string[] { "101", "102" };
        }
        ...
        protected void btnGotoPay_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < selectedDataKeys.Count; i++)
            {
                // 数量输入框属于当前控件行，只能按行位置读取；先用稳定行ID找到当前控件行。
                ...
                sb.AppendFormat("<li>{0}（{1}）</li>", selectedDataKeys[i][2], tbxNumber.Text);
            }
            sb.Append("</ol><hr/>");

            sb.AppendFormat("共 {0} 件商品，总计 ¥{1}", Request.Form["TOTAL_NUMBER"], Request.Form["TOTAL_PRICE"]);

            // 这里拼的是 HTML（有序列表 + 分隔线），要按可信 HTML 传，否则标签会被转义、原样显示给用户
