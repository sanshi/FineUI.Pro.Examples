...
namespace FineUI.Pro.Examples.grid
{
    public partial class grid_edit_save_auto : PageBase
    {
        ...
        {
            if (e.EventName == "AutoSave")
            {
                UpdateAllUserInputData();

                // 输出提示信息
                PageContext.RegisterStartupScript(String.Format("F.notify({0});", JsHelper.Enquote("自动保存于" + DateTime.Now.ToString())));
            }
        }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            // 输出提示信息
            PageContext.RegisterStartupScript(String.Format("F.notify({0});", JsHelper.Enquote("手工保存于" + DateTime.Now.ToString())));
        }
