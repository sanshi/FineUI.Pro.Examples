...
namespace FineUI.Pro.Examples.button
{
    public partial class button_menu : PageBase
    {
        ...
        {
            Button1.Menu.Items.Clear();
            foreach (var menu in CreateMenuItems())
            {
                Button1.Menu.Items.Add(menu);
            }
        }

        private static List<BaseMenuItem> CreateMenuItems()
        {
            List<BaseMenuItem> menus = new List<BaseMenuItem>();
