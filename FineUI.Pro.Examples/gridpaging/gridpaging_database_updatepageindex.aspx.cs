...
{
    public partial class gridpaging_database_updatepageindex : PageBase
    {
        ...
        protected void btnUpdatePageIndex_Click(object sender, EventArgs e)
        {
            // 1.设置总项数（特别注意：数据库分页一定要设置总记录数RecordCount）
            Grid1.RecordCount = GetTotalCount();

            // 2. 设置当前分页索引（最后一页）
            var lastPageIndex = GetPageCount(pageSize: Grid1.PageSize) - 1;
            Grid1.PageIndex = lastPageIndex;

            // 3.获取当前分页数据
            DataTable table = GetPagedDataTable(lastPageIndex, Grid1.PageSize);
