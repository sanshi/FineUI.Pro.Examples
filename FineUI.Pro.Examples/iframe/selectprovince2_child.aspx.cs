...
namespace FineUI.Pro.Examples.iframe
{
    public partial class selectprovince2_child : PageBase
    {
        ...
        {
            if (e.EventName == "SelectProvince")
            {
                PageContext.RegisterStartupScript("F.getActiveWindow().window.selectProvince(" +
                    JsHelper.Enquote(e.EventArguments) + ");" + ActiveWindow.GetHideReference());
            }
        }
