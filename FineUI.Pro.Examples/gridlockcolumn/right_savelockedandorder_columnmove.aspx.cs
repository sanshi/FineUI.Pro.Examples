...
{
    public partial class right_savelockedandorder_columnmove : PageBase
    {
            ...
            if (!IsPostBack)
            {
                ...
                foreach (JObject column in savedColumns)
                {
                    ...
                    if (foundColumn != null)
                    {
                        ...
                        // LockedPosition可能不存在
                        string lockedPosition = column.Value<string>("LockedPosition");
                        if(!String.IsNullOrEmpty(lockedPosition))
                        {
                            foundColumn.LockedPosition = (LockedPosition)Enum.Parse(typeof(LockedPosition), lockedPosition, true);
                        }
                    }
                    order++;
                }
        ...
        private static readonly string KEY_FOR_DATASOURCE_SESSION = "gridlockcolumn.grid_lockcolumn_savelockedandorder_columnmove_right";
