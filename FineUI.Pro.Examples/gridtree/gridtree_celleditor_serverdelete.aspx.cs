...
{
    public partial class gridtree_celleditor_serverdelete : PageBase
    {
        ...
        {
            string[] selectedRowIDs = Grid1.SelectedRowIDArray;
            if (selectedRowIDs.Length > 0)
            {
                foreach (string selectedRowID in selectedRowIDs)
                {
                    int rowID = Convert.ToInt32(selectedRowID);
                    DeleteRowByIDWithChildren(GetSourceData(), rowID);
                }
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            {
                int rowID = Convert.ToInt32(e.RowID);
                DeleteRowByIDWithChildren(GetSourceData(), rowID);
        ...
        private static readonly string KEY_FOR_DATASOURCE_SESSION = "datatable_for_gridtree_celleditor_serverdelete";
        ...
        {
            DataRow found = FindRowByID(GetSourceData(), rowID);
            if (found != null)
            {
                // 删除本行
